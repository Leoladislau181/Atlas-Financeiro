...
export function Layout({ children, activeTab, setActiveTab, onNewLancamento, onProfileClick, user }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] dark:bg-gray-950 font-sans text-gray-900 dark:text-gray-100 pb-24 sm:pb-0 transition-colors duration-200">
      ...
      {/* Floating Premium Banner for Mobile Free Users */}
      {!userIsPremium && (
        <div 
          onClick={() => setActiveTab('premium')}
          className="sm:hidden sticky top-16 z-30 bg-gradient-to-r from-amber-500 to-amber-600 text-white px-4 py-3 flex items-center justify-between cursor-pointer shadow-md"
        >
          <div className="flex items-center gap-2">
            <Star className="h-5 w-5 fill-current text-white" />
            <span className="text-sm font-bold">Seja Premium</span>
          </div>
          <span className="text-xs bg-white/20 px-3 py-1.5 rounded-full font-bold uppercase tracking-wider">Assinar</span>
        </div>
      )}
